...
export interface IngestJob { id:string; state:IngestState; targetDir:string; libraryId:string|null; createdAt:number; updatedAt:number; items:IngestJobItem[] }
...
export class IngestStore {
  ...
  constructor(dbPath = process.env.DB_PATH ?? path.join(process.env.DATA_DIR ?? path.join(process.cwd(),'data'), 'curator.db')) {
    ...
    this.db.exec(`CREATE TABLE IF NOT EXISTS ingest_jobs(id TEXT PRIMARY KEY,state TEXT NOT NULL,target_dir TEXT NOT NULL,library_id TEXT,created_at INTEGER NOT NULL,updated_at INTEGER NOT NULL);
  ...
  create(targetDir:string, libraryId?:string): string { const id=randomUUID(), now=Date.now(); this.db.prepare('INSERT INTO ingest_jobs VALUES(?,?,?,?,?,?)').run(id,'discovered',targetDir,libraryId??null,now,now); return id; }
  ...
  private map(r:any):IngestJob { const items=(this.db.prepare('SELECT * FROM ingest_job_items WHERE job_id=? ORDER BY updated_at').all(r.id) as any[]).map(i=>({id:i.id,jobId:i.job_id,state:i.state,action:JSON.parse(i.action_json),attempts:i.attempts,error:i.error,absItemId:i.abs_item_id,updatedAt:i.updated_at})); return {id:r.id,state:r.state,targetDir:r.target_dir,libraryId:r.library_id,createdAt:r.created_at,updatedAt:r.updated_at,items}; }

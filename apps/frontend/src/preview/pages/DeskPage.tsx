...
export function DeskPage() {
  ...
  return <div className="v2-page">
    ...
    <div className="v2-bento">
      <section className="v2-card v2-trends"><div className="v2-card-head"><div><span className="v2-kicker"><Sparkles/> Scout intelligence</span><h2>Find what deserves a place next</h2></div><Link to="/preview/scout/trends">View trends <ArrowRight/></Link></div><p>Compare bestseller signals, inspect candidates, and hand selected titles to qBittorrent without leaving the librarian workflow.</p><div className="v2-mini-actions"><Link to="/preview/scout/trends"><Sparkles/> Trending now</Link><Link to="/preview/scout/search"><BookCopy/> Search sources</Link></div></section>
      <section className="v2-card v2-review"><div className="v2-card-head"><span className="v2-kicker warning"><CheckCircle2/> Needs review</span><strong className="v2-big-number">{reviewCount}</strong></div><Link className="v2-metric" to="/preview/curate/tags"><span><Tags/><b>Metadata & tags</b></span><strong>{stats.data?.untaggedBooks ?? "—"}</strong></Link><Link className="v2-metric" to="/preview/curate/collections"><span><BookCopy/><b>Collection proposals</b></span><strong>{proposed}</strong></Link></section>
      <section className="v2-card v2-active"><span className="v2-kicker cyan"><WandSparkles/> Active work</span>{active ? <><div className="v2-progress-title"><div><h2>{active.type}</h2><p>{active.progress.message || active.status}</p></div><strong>{pct}%</strong></div><div className="v2-progress"><i style={{"--progress": `${pct}%`} as React.CSSProperties}/></div><Link className="v2-button v2-button-secondary" to={`/preview/activity/${active.id}`}>View operation</Link></> : <div className="v2-empty-compact"><h2>Everything is quiet</h2><p>No scan, curation, or conversion job is currently running.</p><Link to="/preview/curate/encode">Review M4B candidates</Link></div>}</section>
      <section className="v2-card v2-plan"><span className="v2-kicker"><FolderInput/> Filesystem safety</span><h2>Review before files move</h2><div className="v2-path"><small>Current intake</small><code>…/downloads/author-title/</code></div><ArrowRight className="v2-path-arrow"/><div className="v2-path"><small>Preferred structure</small><code>…/Author/Series/01 - Title/</code></div><Link className="v2-button v2-button-secondary" to="/preview/process/organize">Review proposed changes</Link></section>

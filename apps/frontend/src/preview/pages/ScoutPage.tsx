...
export function ScoutPage({ mode }: { mode: "trends" | "search" | "recommendations" | "intake" }) {
  return <div className="v2-page v2-legacy-surface">
    <div className="v2-page-heading"><div><span className="v2-eyebrow">Scout & Acquire</span><h1>{mode === "trends" ? "Find what belongs next" : mode === "recommendations" ? "Ask your recommendation librarian" : mode === "intake" ? "Review intake conflicts" : "Search acquisition sources"}</h1><p>{mode === "trends" ? "Explore external signals, then investigate and acquire a promising title in one workflow." : mode === "recommendations" ? "Describe what you want and pick any reference books; the librarian on your desk answers from your shelf first, with acquirable titles below." : mode === "intake" ? "New files are shelved automatically. Only duplicates, ambiguous matches and errors land here for a decision." : "Search AudiobookBay and send an approved candidate to qBittorrent."}</p></div>{mode === "intake" ? <span className="v2-live warning"><span className="v2-dot warn"/> Live filesystem</span> : <span className="v2-live"><span className="v2-dot ok"/> Live actions</span>}</div>

...
export function ScoutPage({ mode }: { mode: "trends" | "search" }) {
  return <div className="v2-page v2-legacy-surface">
    ...
    <nav className="v2-section-tabs" aria-label="Scout and acquire sections">
      <NavLink to="/preview/scout/trends" className={({ isActive }) => isActive ? "active" : ""}><Sparkles/><span>Trends & discovery</span></NavLink>
      <NavLink to="/preview/scout/search" className={({ isActive }) => isActive ? "active" : ""}><Search/><span>Search & download</span></NavLink>
    </nav>

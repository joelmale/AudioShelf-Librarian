...
export function LibrarianChatPanel({ bookBasePath = '/library/books', acquisitionSearchPath = '/discover/search' }: { bookBasePath?: string; acquisitionSearchPath?: string } = {}) {
  ...
  const loadList = async (cursor?: string, replace = false): Promise<boolean> => { const request = ++listGeneration.current; setListBusy(true); setListError(null); setListRetry(null); const controller = new AbortController(); requestController.current?.abort(); requestController.current = controller; try { const page = await listLibrarianConversations(20, cursor, controller.signal); if (controller.signal.aborted || request !== listGeneration.current) return false; setConversations((old) => { if (replace) return page.conversations; const seen = new Set(old.map((c) => c.id)); return [...old, ...page.conversations.filter((c) => !seen.has(c.id))]; }); setListCursor(page.nextCursor); return true; } catch (error) { if (!controller.signal.aborted && request === listGeneration.current) { setListError(error instanceof Error ? error.message : 'Could not load conversation history.'); setListRetry({ ...(cursor ? { cursor } : {}), replace }); } return false; } finally { if (request === listGeneration.current) setListBusy(false); } };

  ...
  return (
    <div className="glass-panel progress-container" style={{ marginTop: '24px' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', color: 'var(--text-secondary)', fontSize: '0.9rem' }}>
        ...
        <span>{statusText === 'Discovering' ? '...' : `${Math.round(progress)}%`}</span>
      </div>
      ...
      <div style={{
        ...
        <div style={{
          width: statusText === 'Discovering' ? '50%' : `${progress}%`,
          height: '100%',
          background: 'linear-gradient(90deg, var(--primary-accent), var(--secondary-accent))',
          boxShadow: '0 0 10px var(--secondary-glow)',
          transition: statusText === 'Discovering' ? 'none' : 'width 0.3s cubic-bezier(0.16, 1, 0.3, 1)',
          animation: statusText === 'Discovering' ? 'scanSweep 1.5s infinite linear' : 'none'
        }} />
      </div>
      ...
      <div style={{ 
        ...
        <div style={{ 
          ...
          {currentFile ? (statusText === 'Discovering' ? `Discovering: ${currentFile}` : `Scanning: ${currentFile}`) : 'Ready...'}
        </div>
        
        {(statusText === 'Scanning' || statusText === 'Discovering') && (
          <button 
